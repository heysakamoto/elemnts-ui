...
const treeViewRecipeSlotFns = /* @__PURE__ */ treeViewRecipeSlotNames.map(([slotName, slotKey]) => [slotName, createRecipe(slotKey, treeViewRecipeDefaultVariants, getSlotCompoundVariant(treeViewRecipeCompoundVariants, slotName))])

const treeViewRecipeFn = memo((props = {}) => {
  return Object.fromEntries(treeViewRecipeSlotFns.map(([slotName, slotFn]) => [slotName, slotFn.recipeFn(props)]))
})

const treeViewRecipeVariantKeys = []
const getVariantProps = (variants) => ({ ...treeViewRecipeDefaultVariants, ...compact(variants) })
