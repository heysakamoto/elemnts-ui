...
const splitterRecipeSlotNames = [
	...
	["resizeTriggerIndicator", "splitter__resizeTriggerIndicator"],
];
const splitterRecipeSlotFns = /* @__PURE__ */ splitterRecipeSlotNames.map(
	([slotName, slotKey]) => [
		slotName,
		createRecipe(
			slotKey,
			splitterRecipeDefaultVariants,
			getSlotCompoundVariant(splitterRecipeCompoundVariants, slotName),
		),
	],
);
...
const splitterRecipeFn = memo((props = {}) => {
	return Object.fromEntries(
		splitterRecipeSlotFns.map(([slotName, slotFn]) => [
			slotName,
