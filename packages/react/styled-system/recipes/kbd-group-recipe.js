...
const kbdGroupRecipeSlotNames = [
	["root", "kbd-group__root"],
	["separator", "kbd-group__separator"],
];
const kbdGroupRecipeSlotFns = /* @__PURE__ */ kbdGroupRecipeSlotNames.map(
	([slotName, slotKey]) => [
		slotName,
		createRecipe(
			slotKey,
			kbdGroupRecipeDefaultVariants,
			getSlotCompoundVariant(kbdGroupRecipeCompoundVariants, slotName),
		),
	],
);
...
const kbdGroupRecipeFn = memo((props = {}) => {
	return Object.fromEntries(
		kbdGroupRecipeSlotFns.map(([slotName, slotFn]) => [
			slotName,
