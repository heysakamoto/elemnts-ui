...
const dialogRecipeSlotFns = /* @__PURE__ */ dialogRecipeSlotNames.map(
	([slotName, slotKey]) => [
		slotName,
		createRecipe(
			slotKey,
			dialogRecipeDefaultVariants,
			getSlotCompoundVariant(dialogRecipeCompoundVariants, slotName),
		),
	],
);
...
const dialogRecipeFn = memo((props = {}) => {
	return Object.fromEntries(
		dialogRecipeSlotFns.map(([slotName, slotFn]) => [
			slotName,
...
const dialogRecipeVariantKeys = ["size", "placement"];
