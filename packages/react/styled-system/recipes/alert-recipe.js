...
const alertRecipeSlotNames = [
	...
	["control", "alert__control"],
];
const alertRecipeSlotFns = /* @__PURE__ */ alertRecipeSlotNames.map(
	([slotName, slotKey]) => [
		slotName,
		createRecipe(
			slotKey,
			alertRecipeDefaultVariants,
			getSlotCompoundVariant(alertRecipeCompoundVariants, slotName),
		),
	],
);
...
const alertRecipeFn = memo((props = {}) => {
	return Object.fromEntries(
		alertRecipeSlotFns.map(([slotName, slotFn]) => [
			slotName,
...
const alertRecipeVariantKeys = ["status", "variant"];
