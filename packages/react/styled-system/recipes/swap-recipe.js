...
const swapRecipeSlotNames = [
	["root", "swap__root"],
	["indicator", "swap__indicator"],
];
const swapRecipeSlotFns = /* @__PURE__ */ swapRecipeSlotNames.map(
	([slotName, slotKey]) => [
		slotName,
		createRecipe(
			slotKey,
			swapRecipeDefaultVariants,
			getSlotCompoundVariant(swapRecipeCompoundVariants, slotName),
		),
	],
);
...
const swapRecipeFn = memo((props = {}) => {
	return Object.fromEntries(
		swapRecipeSlotFns.map(([slotName, slotFn]) => [
			slotName,
...
const swapRecipeVariantKeys = ["mode"];
