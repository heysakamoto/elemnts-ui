...
const tableRecipeSlotNames = [
	...
	["columnHeader", "table__columnHeader"],
	["columnGroup", "table__columnGroup"],
];
const tableRecipeSlotFns = /* @__PURE__ */ tableRecipeSlotNames.map(
	([slotName, slotKey]) => [
		slotName,
		createRecipe(
			slotKey,
			tableRecipeDefaultVariants,
			getSlotCompoundVariant(tableRecipeCompoundVariants, slotName),
		),
	],
);
...
const tableRecipeFn = memo((props = {}) => {
	return Object.fromEntries(
		tableRecipeSlotFns.map(([slotName, slotFn]) => [
			slotName,
