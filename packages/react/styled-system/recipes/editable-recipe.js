...
const editableRecipeSlotFns = /* @__PURE__ */ editableRecipeSlotNames.map(([slotName, slotKey]) => [slotName, createRecipe(slotKey, editableRecipeDefaultVariants, getSlotCompoundVariant(editableRecipeCompoundVariants, slotName))])

const editableRecipeFn = memo((props = {}) => {
  return Object.fromEntries(editableRecipeSlotFns.map(([slotName, slotFn]) => [slotName, slotFn.recipeFn(props)]))
})

const editableRecipeVariantKeys = []
const getVariantProps = (variants) => ({ ...editableRecipeDefaultVariants, ...compact(variants) })
