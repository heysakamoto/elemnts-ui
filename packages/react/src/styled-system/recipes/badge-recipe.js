...
const badgeRecipeSlotNames = [
	["root", "badge__root"],
	["indicator", "badge__indicator"],
];
const badgeRecipeSlotFns = /* @__PURE__ */ badgeRecipeSlotNames.map(
	([slotName, slotKey]) => [
		slotName,
		createRecipe(
			slotKey,
			badgeRecipeDefaultVariants,
			getSlotCompoundVariant(badgeRecipeCompoundVariants, slotName),
		),
	],
);
...
const badgeRecipeFn = memo((props = {}) => {
	return Object.fromEntries(
		badgeRecipeSlotFns.map(([slotName, slotFn]) => [
			slotName,
...
const badgeRecipeVariantKeys = ["size", "placement", "variant"];
