...
const tabsRecipeSlotFns = /* @__PURE__ */ tabsRecipeSlotNames.map(
	([slotName, slotKey]) => [
		slotName,
		createRecipe(
			slotKey,
			tabsRecipeDefaultVariants,
			getSlotCompoundVariant(tabsRecipeCompoundVariants, slotName),
		),
	],
);
...
const tabsRecipeFn = memo((props = {}) => {
	return Object.fromEntries(
		tabsRecipeSlotFns.map(([slotName, slotFn]) => [
			slotName,
...
const tabsRecipeVariantKeys = ["size", "variant"];
