...
const selectRecipeSlotNames = [
	...
	["hiddenSelect", "select__hiddenSelect"],
	["valueText", "select__valueText"],
];
const selectRecipeSlotFns = /* @__PURE__ */ selectRecipeSlotNames.map(
	([slotName, slotKey]) => [
		slotName,
		createRecipe(
			slotKey,
			selectRecipeDefaultVariants,
			getSlotCompoundVariant(selectRecipeCompoundVariants, slotName),
		),
	],
);
...
const selectRecipeFn = memo((props = {}) => {
	return Object.fromEntries(
		selectRecipeSlotFns.map(([slotName, slotFn]) => [
			slotName,
