...
const marqueeRecipeSlotNames = [
	...
	["edge", "marquee__edge"],
	["item", "marquee__item"],
	["viewport", "marquee__viewport"],
];
const marqueeRecipeSlotFns = /* @__PURE__ */ marqueeRecipeSlotNames.map(
	([slotName, slotKey]) => [
		slotName,
		createRecipe(
			slotKey,
			marqueeRecipeDefaultVariants,
			getSlotCompoundVariant(marqueeRecipeCompoundVariants, slotName),
		),
	],
);
...
const marqueeRecipeFn = memo((props = {}) => {
	return Object.fromEntries(
		marqueeRecipeSlotFns.map(([slotName, slotFn]) => [
			slotName,
