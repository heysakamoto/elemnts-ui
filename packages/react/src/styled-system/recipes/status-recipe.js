...
const statusRecipeSlotNames = [
	["root", "status__root"],
	["indicator", "status__indicator"],
	["text", "status__text"],
];
const statusRecipeSlotFns = /* @__PURE__ */ statusRecipeSlotNames.map(
	([slotName, slotKey]) => [
		slotName,
		createRecipe(
			slotKey,
			statusRecipeDefaultVariants,
			getSlotCompoundVariant(statusRecipeCompoundVariants, slotName),
		),
	],
);
...
const statusRecipeFn = memo((props = {}) => {
	return Object.fromEntries(
		statusRecipeSlotFns.map(([slotName, slotFn]) => [
			slotName,
...
const statusRecipeVariantKeys = ["size"];
