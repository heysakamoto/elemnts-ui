...
const sortableRecipeSlotNames = [
	["root", "sortable__root"],
	["itemHandle", "sortable__itemHandle"],
	["item", "sortable__item"],
];
const sortableRecipeSlotFns = /* @__PURE__ */ sortableRecipeSlotNames.map(
	([slotName, slotKey]) => [
		slotName,
		createRecipe(
			slotKey,
			sortableRecipeDefaultVariants,
			getSlotCompoundVariant(sortableRecipeCompoundVariants, slotName),
		),
	],
);
...
const sortableRecipeFn = memo((props = {}) => {
	return Object.fromEntries(
		sortableRecipeSlotFns.map(([slotName, slotFn]) => [
			slotName,
