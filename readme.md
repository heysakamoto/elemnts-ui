...
**Elemnts UI** is a high-performance, accessible component library engineered for speed and scalability. Built on the foundation of [Ark UI](https://ark-ui.com/) and powered by [Panda CSS](https://panda-css.com/), it provides a robust toolkit for building modern, themeable design systems.
...
Elemnts UI relies on **Panda CSS** for its high-performance styling engine. Follow these steps to integrate the library and its design tokens into your project.

---

### 1. Installation

Install the Elemnts UI core components and the Panda CSS development dependencies:
...
Create a `panda.config.ts` file in your project root. This configuration imports the **Elemnts UI base preset** to ensure all component styles are correctly mapped:
...
Install @elemnts-ui/react and start using the available components:
...
Elemnts UI includes a comprehensive suite of primitives:
...
Elemnts UI leverages Panda CSS **recipes** and **tokens**. You can extend the default theme in your `panda.config.ts`:

```typescript
import { defineConfig } from '@pandacss/dev'

export default defineConfig({
  // Extend Elemnts UI tokens here
  theme: {
    extend: {
      tokens: {
        colors: {
...
The Elemnts UI project is organized into the following packages:

- **`packages/core`**: Provides the core presets, colors and design tokens.
- **`packages/react`**: Provides React components and hooks for building styled UI with Elemnts UI.

---

## 🤝 Contributing

We welcome contributions to make Elemnts UI even faster! Please check our [Contributing Guide](./contributing.md) to get started.
