---
title: Theming
description: A step-by-step guide to customizing and extending Moto UI's design system to match your brand.
---
...
Moto UI allows you to extend or override the default theme by adding your own design tokens, semantic tokens, and text styles in the `theme.extend` section of your configuration.
...
Moto UI components are styled using **Recipes** and **Slot Recipes**. You can override these to change the default appearance of any component, such as a `Button` or `Card`.
...
|--------------|-------------|
| Use extend | Always place your customizations inside `theme.extend` to avoid accidentally overwriting the base Moto UI theme. |
...
| Maintain Consistency | Stick to the provided duration and easing tokens to preserve the "Moto" feel across your design system. |
| Organize Your Tokens | Group related tokens (e.g., colors, fonts) logically to make your configuration easier to maintain. |

---

Need help? Check out the [Panda CSS documentation](https://panda-css.com/docs) or reach out to the Moto UI community for support!
