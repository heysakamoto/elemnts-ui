...
description: Easily install and set up Moto UI in your project.
---

To start using Moto UI, follow these steps to set up the library and its styling engine, Panda CSS.
...
export default defineConfig({
  ...
  // 4. Output directory for the generated CSS-in-JS
...
1. Your `panda.config.ts` includes the `src/components` path in the `include` array.
2. You have run `npx panda` (or your build script) to generate the `styled-system` directory.
3. You have imported your `index.css` file at the root of your project.

### Font Issues
Moto UI presets use `--font-sans-default` and `--font-mono-default` variables. If your fonts look off, make sure these are defined either in your `globalVars` in `panda.config.ts` or as standard CSS variables in your `@layer base`.

## Next Steps

- [Composition](/docs/components/concepts/composition): Master polymorphic components with `as` and `asChild`.
- [Colors](/docs/components/concepts/colors): Explore the OKLCH-based semantic color system.
- [Styling](/docs/components/concepts/styling): Learn how to use style props and recipes.
